...
class HeartsGamesInfo(object):
    ...
    class GameState:
        ...
        def json_str(self):
            ...
            work_str = json.dumps(
                {"game": [{"scores": self.scores}, {"players": player_names}]}
            )
    ...
    def play_tricks(players, game_state):
        ...
        for trick in range(0, num_tricks):
            ...
            for player in range(0, num_players):
                ...
                util.debug_out(
                    str(sorted(players[current_player].hand)), debug_level + 100
                )
            ...
            util.debug_out(
                "-==-=- after trick, starter_player = " + str(starter_player),
                debug_level,
            )
    ...
    def play_games(self, players, num_games):
        ...
        print(f"Playing {num_games} game(s) with {len(players)} players.")
        game_results = []
        ...
        for game_num in range(0, num_games):
            ...
            self.game_outfile.write(
                str(game_num) + "\t" + str(util.return_game_info(result))
            )
            self.game_wins[util.game_winner(result.scores)] += 1
            if ((game_num + 1) % 250) == 0:
                percent_wins = win_percent(game_num + 1, self.game_wins)
                print(f"{game_num} \t {percent_wins}")
