...
from pyhearts.game import Face, HeartsCard, Suit, TWO_OF_CLUBS

from faker import Faker


def highest_value_card(cards):
    highest_value = 0
    highest_card = ""
...
class BasePlayer:
    ...
    def __str__(self) -> str:
        ...
        return f"Player {self.name}" " ".join(self.hand)
...
class RandomPlayer(BasePlayer):
    ...
    def play(self, game_state):
        current_hand = game_state["hand"]
        visible_cards = game_state["visible_cards"]
        hearts_broken = game_state["hearts_broken"]
        led_suit = game_state["led_suit"]

        if TWO_OF_CLUBS in current_hand:

            return TWO_OF_CLUBS

        if hearts_broken:
            return random.choice(current_hand)

        while True:
            card_choice = random.choice(current_hand)
            if not (card_choice.suit == Suit.Hearts):
                return card_choice
        # return random.choice(current_hand)

    def _set_name(self):
        fake = Faker()
        return fake.name() + " (Random)"
